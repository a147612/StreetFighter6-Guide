...
- **場地位置圖**：每個情境上方畫出你和對手的相對位置、你背靠的是哪面牆、以及誰倒在地上。
  刻意畫得很簡略 —— 這裡有用的資訊只有相對位置，畫得更細只會既不準確又動用到別人的美術。
- **術語表**（`#/glossary`）：43 個動作的**三語並排**對照，含別稱與英文原詞。
  介面一次只顯示一種語言，所以這是唯一能看到「重ね＝壓起身＝meaty」的地方 ——
  也是讓日文／英文攻略能映射回這份指南的東西。
